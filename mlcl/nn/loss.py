...
class Loss:
    ...
    def __call__(self, y_pred: np.ndarray, y_true: np.ndarray) -> float:
        """
        Calculate the loss between predictions and ground truth values.
...
class MSELoss(Loss):
    ...
    def forward(self, y_pred, y_true):
        ...
        if isinstance(y_pred, Tensor):
            ...
            self.diff = diff  # Save for backward pass
            loss_value = accelerated_ops.mean(accelerated_ops.clip(diff * diff, 0, float('inf')))
            ...
            def _backward(grad=None):
                ...
                self.y_pred.backward(grad_factor)
                
            loss_tensor._backward = _backward
            return loss_tensor
            
        return accelerated_ops.mean(accelerated_ops.clip((y_pred - y_true) * (y_pred - y_true), 0, float('inf')))
    ...
    def forward(self, y_pred, y_true):
        ...
        if isinstance(y_pred, Tensor):
            ...
            def _backward(grad=None):
                ...
                # BCE gradient is: -(y_true/y_pred - (1-y_true)/(1-y_pred))
